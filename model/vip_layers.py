...
class AnyAttention(nn.Module):
    ...
    def forward(self, q=None, k=None, v=None, qpos=None, kpos=None, mask=None, rel_pos=None):
        ...
        VIS = attn.detach()
        # print(VIS.shape)
        # print(attn.shape) Batch x Num_parts x Heads x (patchsize*patchsize)
        #just average out of heads and get Batch x Num_Parts x (patchsize*patchsize)
        #take first any of the part with attn[0][part] and then reshape to patchsize x patchsize
        #then finally reshape to the imagesize x imagesize and visualize
