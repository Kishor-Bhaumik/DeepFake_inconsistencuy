...
class Encoder(nn.Module):
    ...
    def forward(self, feats, parts=None, qpos=None, kpos=None, mask=None):
        ...
        Returns:
            parts: [B, N, C]
        """
        attn_out, VIS = self.enc_attn(q=parts, k=feats, v=feats, qpos=qpos, kpos=kpos, mask=mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, parts=None, part_kpos=None, mask=None, P=0):
        ...
        out, _ = self.attn1(q=x, k=parts, v=parts, kpos=part_kpos, mask=dec_mask)
        out = x + self.drop_path(out)
        out = out + self.drop_path(self.ffn1(out))

        out = rearrange(out, "b (p k) c -> (b p) k c", p=P)
        local_out, _  = self.attn2(q=out, k=out, v=out, mask=mask, rel_pos=self.rel_pos)
...
class ViPBlock(nn.Module):
    ...
    def forward(self, x, parts=None, part_qpos=None, part_kpos=None, mask=None):
        ...
        parts, VIS = self.encoder(x, parts=parts, qpos=part_qpos, mask=mask)
        feats = self.decoder(x, parts=parts, part_kpos=part_kpos, mask=mask, P=P)
        return feats, parts, part_qpos, mask, VIS
    ...
    def forward(self, x, parts=None, mask=None):
        ...
        if len(x.shape) == 5:
            ...
            for i in range(x.shape[1]):
                x_i, H, W, mask = self.proj(x[:,i,:,:,:], mask=mask)
                x_i = self.proj_norm(x_i)
                ori_H, ori_W = H, W
                x_i, mask, H, W = self.to_patch(x_i, self.patch_size, H, W, mask)
                x_ii.append(x_i)
                
            x = torch.cat(x_ii, dim=1)
        ...
        for blk in self.blocks:
            # x: [B, K, P, C]
            x, parts, rpn_qpos, mask, VIS = blk(x,
                                           parts=parts,
                                           part_qpos=rpn_qpos,
                                           part_kpos=rpn_kpos,
                                           mask=mask)
        ...
        if self.last_enc is not None:
            ...
            #HERE rpn_out is THE FINAL REPRESENTATION vector AFTER FINAL ENCODER
            #parts is the representation vector of the exact previous encoder
        else:
            x = rearrange(x, "b (sh sw) (kh kw) c -> b c (sh kh) (sw kw)", kh=self.patch_size, sh=H // self.patch_size)
            x = x[:, :, :ori_H, :ori_W]
            return x, parts, mask, VIS
...
class ViP(nn.Module):
    ...
    def forward(self, x):
        ...
        #if last encoder = true the last stage will be just an transformer ENCODER which will give the 
        # representations as output. 
        for i in range(self.depth):
            layer = getattr(self, "layer_{}".format(i))
            out, rpn_tokens, mask, VIS = layer(out, rpn_tokens, mask=mask)
        return out, rpn_tokens, VIS 

        #out is the output of the last vision transformer encoder (IF last encoder=true) which gives both represtion vectors and 
        #the eqn2 visualization. rpn_tokens is the representation vector of exactly the previous transformer encoder
...
def vip_mobile(pretrained=False, **cfg):
    # [16, 16, 16, 32] chilo num_parts default
    model_cfg = dict(inplanes=64, num_chs=(48, 96, 192, 384), patch_sizes=[8, 7, 7, 7], num_heads=[1, 2, 4, 8],
                     num_enc_heads=[1, 2, 4, 8], num_parts=[16, 16, 16, 16], num_layers=[1, 1, 1, 1], ffn_exp=3,
                     has_last_encoder=False, drop_path=0., **cfg)
...
def vip_tiny(pretrained=False, **cfg):
    model_cfg = dict(inplanes=64, num_chs=(64, 128, 256, 512), patch_sizes=[8, 7, 7, 7], num_heads=[1, 2, 4, 8],
                     ...
                     has_last_encoder=False, drop_path=0.1, **cfg)
    return ViP(pretrained=pretrained, **model_cfg)
...
def vip_medium(pretrained=False, **cfg):
    model_cfg = dict(inplanes=64, num_chs=(96, 192, 384, 768), patch_sizes=[8, 7, 7, 7], num_heads=[3, 6, 12, 24],
                     ...
                     has_last_encoder=True, drop_path=0.2, **cfg)
    return ViP(pretrained=pretrained, **model_cfg)
